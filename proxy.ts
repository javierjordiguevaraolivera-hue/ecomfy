...
export function proxy(request: NextRequest) {
  ...
  if (request.nextUrl.pathname === "/iul-es-jd") {
    const existingHeroVariant = request.cookies.get(IUL_JD_HERO_COOKIE_NAME)?.value;
    const heroVariant: string = FE7_HERO_VARIANTS.includes(
      existingHeroVariant as (typeof FE7_HERO_VARIANTS)[number],
    )
      ? (existingHeroVariant as string)
    ...
    requestHeaders.set(IUL_JD_HERO_HEADER_NAME, heroVariant);

    if (existingHeroVariant !== heroVariant) {
      responseCookies.push({
        name: IUL_JD_HERO_COOKIE_NAME,
        value: heroVariant,
      });
    }
  }
  ...
  if (request.nextUrl.pathname === "/iul-es-jd-an") {
    const existingHeroVariant = request.cookies.get(IUL_JD_AN_HERO_COOKIE_NAME)?.value;
    const heroVariant: string = FE7_HERO_VARIANTS.includes(
      existingHeroVariant as (typeof FE7_HERO_VARIANTS)[number],
    )
      ? (existingHeroVariant as string)
    ...
    requestHeaders.set(IUL_JD_AN_HERO_HEADER_NAME, heroVariant);

    if (existingHeroVariant !== heroVariant) {
      responseCookies.push({
        name: IUL_JD_AN_HERO_COOKIE_NAME,
        value: heroVariant,
      });
    }
  }
  ...
  if (request.nextUrl.pathname === "/iul-es-jd-an3") {
    ...
    requestHeaders.set(IUL_JD_AN3_HERO_HEADER_NAME, heroVariant);

    if (existingHeroVariant !== heroVariant) {
      responseCookies.push({
        name: IUL_JD_AN3_HERO_COOKIE_NAME,
        value: heroVariant,
      });
    }
  }
